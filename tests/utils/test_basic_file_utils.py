...
# ============================================================================
# has_pyproject_toml tests - Positive cases
# ============================================================================

def test_has_pyproject_toml_returns_true_when_file_exists(tmp_path):
    """Verify that has_pyproject_toml returns True when pyproject.toml exists."""
    pyproject_file = tmp_path / "pyproject.toml"
    pyproject_file.write_text("[project]\nname = 'test'\n")

    assert has_pyproject_toml(tmp_path) is True


def test_has_pyproject_toml_accepts_string_path(tmp_path):
    """Verify that has_pyproject_toml accepts string path."""
    pyproject_file = tmp_path / "pyproject.toml"
    pyproject_file.write_text("[project]\nname = 'test'\n")

    assert has_pyproject_toml(str(tmp_path)) is True


def test_has_pyproject_toml_accepts_path_object(tmp_path):
    """Verify that has_pyproject_toml accepts Path object."""
    pyproject_file = tmp_path / "pyproject.toml"
    pyproject_file.write_text("[project]\nname = 'test'\n")
...
# ============================================================================
# has_pyproject_toml tests - Negative cases
# ============================================================================

def test_has_pyproject_toml_returns_false_when_file_missing(tmp_path):
    """Verify that has_pyproject_toml returns False when pyproject.toml doesn't exist."""
    # tmp_path exists but has no pyproject.toml
    assert has_pyproject_toml(tmp_path) is False


def test_has_pyproject_toml_returns_false_when_other_files_exist(tmp_path):
    """Verify that has_pyproject_toml returns False even when other files exist."""
...
def test_has_pyproject_toml_raises_on_none_path():
    """Verify that has_pyproject_toml raises ValueError for None path."""
...
def test_has_pyproject_toml_raises_on_empty_string():
    """Verify that has_pyproject_toml raises ValueError for empty string."""
    with pytest.raises(ValueError, match="cannot be empty"):
        has_pyproject_toml("")


def test_has_pyproject_toml_raises_on_whitespace_string():
    """Verify that has_pyproject_toml raises ValueError for whitespace string."""
    with pytest.raises(ValueError, match="cannot be empty"):
        has_pyproject_toml("   ")


def test_has_pyproject_toml_raises_on_invalid_type():
    """Verify that has_pyproject_toml raises TypeError for invalid type."""
    with pytest.raises(TypeError, match="must be a string or Path"):
        has_pyproject_toml(123)


def test_has_pyproject_toml_raises_on_nonexistent_folder(tmp_path):
    """Verify that has_pyproject_toml raises ValueError for nonexistent folder."""
    nonexistent = tmp_path / "does_not_exist"

    with pytest.raises(ValueError, match="does not exist"):
        has_pyproject_toml(nonexistent)


def test_has_pyproject_toml_raises_on_file_path(tmp_path):
    """Verify that has_pyproject_toml raises ValueError when path is a file."""
    test_file = tmp_path / "test.txt"
    test_file.write_text("content")

    with pytest.raises(ValueError, match="not a directory"):
        has_pyproject_toml(test_file)


# ============================================================================
# has_pyproject_toml tests - Edge cases
# ============================================================================

def test_has_pyproject_toml_returns_false_when_pyproject_is_directory(tmp_path):
    """Verify that has_pyproject_toml returns False when pyproject.toml is a directory."""
...
def test_has_pyproject_toml_in_nested_project(tmp_path):
    """Verify that has_pyproject_toml only checks the specified directory."""
    ...
    assert has_pyproject_toml(nested) is True


def test_has_pyproject_toml_with_empty_file(tmp_path):
    """Verify that has_pyproject_toml returns True even for empty pyproject.toml."""
    pyproject_file = tmp_path / "pyproject.toml"
...
def test_has_pyproject_toml_with_symlink_to_directory(tmp_path):
    """Verify that has_pyproject_toml works with symlinked directories."""
    ...
    assert has_pyproject_toml(symlink_dir) is True


def test_has_pyproject_toml_with_symlink_file(tmp_path):
    """Verify that has_pyproject_toml detects symlinked pyproject.toml files."""
